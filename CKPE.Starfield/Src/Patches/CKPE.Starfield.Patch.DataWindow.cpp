	...
	namespace Starfield
	{
		namespace Patch
		{
			...
			bool DataWindow::DoActive(Common::RelocatorDB::PatchDB* db) noexcept(true)
			{
				...
				*(std::uintptr_t*)&_oldWndProc =
					Detours::DetourClassJump(__CKPE_OFFSET(0), (std::uintptr_t)&HKWndProc);

				return true;
			}

			LRESULT CALLBACK DataWindow::HKWndProc(HWND Hwnd, UINT Message, WPARAM wParam, LPARAM lParam) noexcept(true)
			{
				...
				const char* lpstrFileName, const char* lpstrTypeName) noexcept(true)
			{
				...
				if ((type & EditorAPI::TESFile::FILE_RECORD_LIGHT) == EditorAPI::TESFile::FILE_RECORD_LIGHT)
					canvas.FillWithTransparent(rc, RGB(0, 255, 0), 10);
				else if ((type & EditorAPI::TESFile::FILE_RECORD_MID) == EditorAPI::TESFile::FILE_RECORD_MID)
